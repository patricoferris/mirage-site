...
module Make 
  ...
  let sync ~gs ~conduit ~resolver =
    let repo = Store.repo gs.store in 
    Store.Repo.branches repo >>= Lwt_list.iter_p (Store.Branch.remove repo) >>= fun _ -> 
    Store.Repo.close (Store.repo gs.store) >>= fun _ ->
    init_blog_store ~resolver ~conduit ~uri:(Key_gen.git_remote ()) >>= fun ngs ->
    Store.list ngs.store [] >|= List.iter (fun (step, kind) ->
        match kind with
        | `Contents -> Log.info (fun f -> f "FILE %s\n" step)
        | `Node -> Log.info (fun f -> f "DIR %s\n" step))  
     >>= fun _ ->        
    Lwt.catch
      (fun () ->
         Sync.pull_exn ngs.store ngs.remote `Set >|= fun _ ->
         (gs.store <- ngs.store))
      (fun e ->
         Log.warn (fun f -> f "failed pull %a" Fmt.exn e);
         Lwt.return (failwith "Error"))
